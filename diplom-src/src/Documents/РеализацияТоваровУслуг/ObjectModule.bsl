...
Процедура ВыполнитьАвтозаполнение() Экспорт
	...
	Если СуммаАбонентскойПлаты > 0 Тогда
		...
		Услуга.Количество = 1;
		Услуга.Цена = СуммаАбонентскойПлаты;
		Услуга.Сумма = СуммаАбонентскойПлаты;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ВКМ_ВыполненныеКлиентуРаботыОбороты.СуммаКОплатеПриход / ВКМ_ВыполненныеКлиентуРаботыОбороты.КоличествоЧасовПриход, 0) КАК Цена,
